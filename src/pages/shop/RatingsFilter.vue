  ...
  <div class="ratings-filter">
    <div class="rating-type border-1px">
      ...
      <span class="block" :class="{active: selectType===0}" @click="setType(0)">
        推荐
        <span class="count">{{recommendRatings}}</span>
      </span>
      <span class="block" :class="{active: selectType===1}" @click="setType(1)">
        吐槽
        <span class="count">{{totalRatings-recommendRatings}}</span>
      </span>
    </div>
    <div class="switch" :class="{on: onlyText}" @click="toggleShow">
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    computed: {
      ...mapGetters(['totalRatings', 'recommendRatings'])
    },
    mounted() {
    },
    methods: {
      ...
      toggleShow () {
        this.$eventBus.$emit('checkOnlyText')
      }
    },
